...
ProcKill(int code, Datum arg)
{
	...
	if (proc->lockGroupLeader != NULL)
	{
		 ...
		 * a controller can attach a PID-scoped "wait" action per name.  The
		 * prockill_race wait callback deliberately does not depend on
		 * procLatch / MyLatch / MyProc->wait_event_info -- all of which
		 * are already torn down by this point -- and polls a shared flag
		 * instead.
		 */
